...
async function checkBilling(data) {
  ...
  await page.getByRole('link', { name: '요양비청구내역조회', exact: true }).click();
  ...
  const [response] = await Promise.all([
    // 특정 URL에 대한 응답을 기다림
    page.waitForResponse(response => response.url() === 'https://medicare.nhis.or.kr/portal/bk/c/230/selectBcbnfDmdResultList.do'),
