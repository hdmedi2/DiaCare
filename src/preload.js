...
window.addEventListener("DOMContentLoaded", () => {
    ...
    if (url.includes("/pharm/diabetes/calc-list-view") || url.includes("/pharm/diabetes/calc-list")) {
        ...
        document.addEventListener('click', function(event) {
            console.log("click1")
            // 예시: 공통 클래스 'future-button'을 가진 요소를 처리하는 경우
            const button_hometax_billing = event.target.closest('#autoTaxInvoiceBillBtn');
            console.log("click2")
            if (button_hometax_billing) {
                console.log('클래스가 future-button인 요소가 클릭되었습니다! ID:', button_hometax_billing.id);
                /* 2024.12.18
                 홈택스 신고자료 파일명 설정
               */
        ...
        if (!isEmpty(button_hometax_billing)) {
            button_hometax_billing.addEventListener('click', () => {s
            });
        } else {
